<template>
    <el-form :model="form" :rules="rules" ref="form" label-width="100px" status-icon>
        <el-form-item label="旧密码" prop="pw">
  ...
  export default{
    ...
    data(){
      return {
        ...
        rules: {
          //req key and rules key must same
          pw: [
    ...
    methods:{
      submit(){
        this.$refs.form.validate((valid) => {
          if (valid) {
            this.$axiosPlus('user/change-fund-password',
              zipObject(['old','new','confirm'].map(_ => `${_}_password`))(Object.values(this.req))
            ,() => {
              this.$message({
                message: '修改成功！',
                type: 'success',
