    ...
    submit(){
      this.getUser(() => {
        if(this.quota) {
          ...
          this.$axiosPlus('user-register-link/create',{prize_group,use_times,expired_date:date.toISOString().slice(0,10)},registerItem => {
            this.$message({
              message:`
                恭喜，注册链接创建成功！<br>
                链接：${``}<br>
            ...
            this.set([registerItem].concat(register))
