...
export default {
  ...
  methods:{
    login(){
      const {ip} = this
      this.$axiosPlus('/user/login',{ip,...this.req},data => {
        if(data.message){
          //errorCode 100102 不存在的用户
          // 100131 密码错误
          this.errorMsg = data.message
        }else if(data.user_token){
          const {user_token:{token,expired_at:expires},user,user_balance_account:bal,user_role:role} = data
          this.$axios.setHeader('Token',token)
          this.setUser({token,expires,role,...user})
          this.setBal(bal)
          this.errorMsg = this.username + '已登錄' + new Date().toLocaleString()
        }
      },{errCb:true,showError:false})
    },
    testApi(){
      const {apiUri,param} = this
      this.$axiosPlus(apiUri,param ? JSON.parse(param.replace(/\s/g,'')) : {},data => this.apiRes = JSON.stringify(data))

      this.testObj = zipObject(['a','b'])(['sss','bbb'])
      // this.testObj = zipObject(['a','b'],['sss','bbb'])
    },
    async openAccount(){
      const {data:{mini_prize_group,max_prize_group}} = await this.$axios.$post('user/find-child-prize-group-range')
      this.newAccount.prize_group = mini_prize_group + Math.floor(Math.random() * 10)
      this.$axiosPlus('user/open-account',this.newAccount)
    },
    ...mapMutations({
      setUser:'setUser',
      setBal:'pay/setBal'
