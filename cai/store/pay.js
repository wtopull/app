...
export const actions = {
  async getBal({commit},callback = () => {}){
    commit('setBal',(await this.app.$axios.$post('user-balance-account/find-info')).data)
    callback()
  },
  ...
  getBankCards({commit,state:{bankCards}}){
    //f5 refresh this.app.$axiosPlus is not a function
    // return this.app.$axiosPlus('user-bank-card/get',data => {
    ...
    return this.app.$axios.$post('user-bank-card/get').then(({data}) => data && data.user_bank_cards && commit('setBankCards',data.user_bank_cards.map(_ => ({..._.content}))))
  }
}
